...
/*   Updated: 2025/04/12 22:21:54 by chlee2           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/minishell.h"

void	finalize_token_helper(t_shell *shell, char **ct, char *s)
{
	...
	if (shell->expanded_ct_flag)
	{
		...
		new_expanded_tokens = ft_calloc((new_expanded_token_counts + 2),
				sizeof(char *));
...
void	finalize_token(t_shell *shell, char **ct, int *token_count, char *s)
{
	...
	if (*ct)
	{
		...
		finalize_token_helper(shell, ct, s);
		(*token_count)++;
		*ct = NULL;
	}
}
